...
function draw(){
   ...
   background(bgImg)
    ...
    if(Wiz.score >= 1 && level == 2){
        level = 3
        Wiz.reset()
    }
    if(level == 3){
        background(bg3Img)
        health = createSprite(630,75,healthBar*4,10)
        health.shapeColor = "red"
        dragon = createSprite(1000,400,100,100)
        dragon.addImage(dragonImg)
        dragon.scale = 0.7
        Wiz.play()
        Wiz.zombieGroup.destroyEach()
        Wiz.skeletonGroup.destroyEach()
